...
export default function LoginPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "signup") {
      // TODO: wire to Supabase signUp when ready
      setError("Account creation coming soon — try Demo for now.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0f0f0f] flex flex-col items-center justify-center px-4 py-12 transition-colors duration-200">
      <div className="w-full max-w-sm">
        ...
        {/* Mode toggle */}
        <div className="flex items-center bg-gray-100 dark:bg-[#1c1c1c] rounded-full p-1 mb-6">
          <button
            onClick={() => switchMode("login")}
            className={`flex-1 py-2 rounded-full text-sm font-semibold transition-colors ${
              mode === "login"
                ? "bg-white dark:bg-[#2a2a2a] text-gray-900 dark:text-white shadow-sm"
                : "text-gray-400 dark:text-[#666] hover:text-gray-600 dark:hover:text-[#999]"
            }`}
          >
            Log In
          </button>
        ...
        <button
          type="button"
          disabled
          className="w-full flex items-center justify-center gap-3 rounded-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#1c1c1c] py-3 text-sm font-semibold text-gray-700 dark:text-[#ccc] opacity-60 cursor-not-allowed"
        >
          <GoogleIcon />
        ...
        <div className="flex items-center gap-3 my-5">
          ...
          <span className="text-xs text-gray-300 dark:text-[#555]">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          {/* Error / info */}
          {error && (
            <p className={`text-sm text-center rounded-xl px-4 py-2.5 border ${
        ...
        <button
          onClick={handleDemo}
          className="mt-3 w-full rounded-full border border-gray-200 dark:border-[#333] py-3 text-sm font-semibold text-gray-700 dark:text-[#ccc] hover:bg-gray-50 dark:hover:bg-[#1c1c1c] transition-colors"
        >
